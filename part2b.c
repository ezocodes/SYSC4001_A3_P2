...
void load_rubric(const char *rubric_file, shared_data_t *shm) {
    ...
    // if fewer than 5 lines, fill defaults
...
void load_exam_from_file(shared_data_t *shm, int exam_index) {
    ...
    // reset marking flags for new exam
...
void review_rubric(int ta_id, shared_data_t *shm) {
    for (int q = 0; q < NUM_QUESTIONS; q++) {
        ...
        int need_correction = rand() % 2;

        if (need_correction) {
            // lock the rubric so only one TA changes it at a time
            sem_wait(&shm->rubric_sem);

            // re-check in case another TA already changed it
            old_letter = shm->rubric[q];
            char new_letter = old_letter + 1;
            shm->rubric[q] = new_letter;

            sem_post(&shm->rubric_sem);

            printf("TA %d: updated Q%d from %c to %c\n",
...
void mark_exam_questions(int ta_id, shared_data_t *shm) {
    while (1) {
        ...
        // pick a question (only one TA can be here at a time)
        ...
        for (int i = 0; i < NUM_QUESTIONS; i++) {
            if (shm->question_marked[i] == 0) {
                all_done = 0;
                q_to_mark = i;   // take the first free one
                break;
            }
        }
...
void ta_process(int ta_id, shared_data_t *shm) {
    ...
    // keep working on exams until stop flag is set
    while (!shm->stop) {
        ...
        // load next exam (2b: protected by exam_sem)
...
int main(int argc, char *argv[]) {
    ...
    // init semaphores (shared between processes)
    if (sem_init(&shm->rubric_sem, 1, 1) == -1) {
        perror("sem_init rubric_sem");
        exit(1);
    }
    if (sem_init(&shm->question_sem, 1, 1) == -1) {
        perror("sem_init question_sem");
        exit(1);
    }
    if (sem_init(&shm->exam_sem, 1, 1) == -1) {
        perror("sem_init exam_sem");
        exit(1);
    }
    ...
    for (int i = 0; i < num_TAs; i++) {
        ...
        // parent keeps looping to fork the rest
    }
    ...
    // cleanup semaphores (only parent)
